...
if wget --help 2>&1 | grep -q -- '--show-progress'; then
    ...
    # MODIFIED 2026-04-05: allow CentOS7-era wget builds that lack --show-progress.
    # ORIGINAL:
    # download commands unconditionally used --show-progress and failed on older wget.
    # MODIFIED:
    # degrade to an empty flag so the same script still works on remote CentOS7 login nodes.
    WGET_PROGRESS_FLAG=""
fi
...
download_pmpnn_weights() {
    ...
    for file in "${ca_files[@]}"; do
        if [ -f "$file" ] && [ -s "$file" ]; then
            ...
            wget -q ${WGET_PROGRESS_FLAG} "https://github.com/dauparas/ProteinMPNN/raw/8907e6671bfbfc92303b5f79c4b5e6ce47cdef57/ca_model_weights/$file" -O "$file"
    ...
    for file in "${vanilla_files[@]}"; do
        if [ -f "$file" ] && [ -s "$file" ]; then
            ...
            print_info "  Downloading $file..."
            wget -q ${WGET_PROGRESS_FLAG} "https://github.com/dauparas/ProteinMPNN/raw/8907e6671bfbfc92303b5f79c4b5e6ce47cdef57/vanilla_model_weights/$file" -O "$file"
            downloaded=$((downloaded + 1))
        fi
    done
    cd ..
...
download_af2_weights() {
    ...
    wget ${WGET_PROGRESS_FLAG} -O "${params_file}" "https://storage.googleapis.com/alphafold/alphafold_params_2022-12-06.tar" || { 
...
download_complexa_weights() {
    ...
    if [ ! -f "$fm_ckpt" ] || [ ! -s "$fm_ckpt" ]; then
        ...
        wget --content-disposition ${WGET_PROGRESS_FLAG} -O "$fm_ckpt" \
    ...
    if [ ! -f "$ae_ckpt" ] || [ ! -s "$ae_ckpt" ]; then
        ...
        wget --content-disposition ${WGET_PROGRESS_FLAG} -O "$ae_ckpt" \
...
download_rf3_weights() {
    ...
    wget ${WGET_PROGRESS_FLAG} -O "$ckpt_file" \
