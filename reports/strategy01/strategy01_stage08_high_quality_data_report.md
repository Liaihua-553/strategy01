# Strategy01 Stage08 高质量多状态复合物数据审计与下一步微调报告

生成时间：2026-04-30 10:55:12
...
```text
/data/kfliao/general_model/strategy_repos/Strategy01_complexa_multistate_benchmarkbase/data/strategy01/stage07_validation_expand/V_exact_manifest.json
/data/kfliao/general_model/strategy_repos/Strategy01_complexa_multistate_benchmarkbase/data/strategy01/stage06_validation/V_exact_manifest.json
```

输出目录：`data/strategy01/stage08_high_quality_dataset`

状态：`passed`

总样本数：`64`

按类别统计：V_exact_main: 48, invalid_or_auxiliary: 10, V_exact_challenge: 6

按 exact kind 统计：exact_experimental_multimodel_nmr: 64

按 motion bin 统计：small_1_2p5: 13, medium_2p5_5: 21, too_large_gt10: 9, large_5_8: 14, challenge_8_10: 7

按 source_db 统计：rcsb_nmr: 64

按类别 family 数：V_exact_main: 41, invalid_or_auxiliary: 9, V_exact_challenge: 6
...
状态：`no_cross_entry_candidates_in_smoke_window`

查询条目数：`200`

有可用 chain pair 的条目数：`31`

序列 group 数：`22`

至少两个 entry 的 group 数：`3`

cross-entry 候选数：`0`

候选 manifest：`data/strategy01/stage08_high_quality_dataset/cross_entry_candidates_manifest.json`

被拒绝的跨条目同序列 group 诊断：

- `7e722986fb49__8b3c46f11cf2`：entries=1a0n,1azg，reason=motion_above_max，motion=16.3062 Å，persistent_anchors=9，min_contacts=11
- `340ad9d1b636__1bfab6b80463`：entries=1ai0,1aiy，reason=motion_above_max，motion=13.5591 Å，persistent_anchors=31，min_contacts=31
- `645a67df9384__0ba17b4132d1`：entries=1cqg,1cqh，reason=motion_below_min，motion=0.189 Å，persistent_anchors=25，min_contacts=25
...
实际执行中还出现了一次 Windows PowerShell 远程脚本 CRLF 问题：早期 `bash` 续行参数混入 `\r`，导致 summary 文件被写成 `stage08_exact_audit_summary.json\r` 和 `stage08_cross_entry_mining_smoke_summary.json\r`，报告脚本因此读不到 summary 并显示 `missing`。修正方式是用远程 Python 扫描并重命名带 `\r` 的文件名，之后统一用 LF 写远程临时脚本并重新生成报告。修正后两个 summary 均可正常读取，报告状态分别为 `passed` 和 `no_cross_entry_candidates_in_smoke_window`。
