...
## Stage11G sampled-latent cache 诊断与代码科学审查

详见独立报告：`reports/strategy01/strategy01_stage11g_code_science_audit_report.md`。

核心结论：当前代码可运行，但还没有实现最终科学目标。Stage11G 证明 exact AE latent 可以近乎恢复真实 binder 序列，而 sampled local latent 只能达到约 0.44 mean identity；因此瓶颈不是 AE decoder 或最终 sequence readout，而是 sampled `local_latents_states` 分布本身。直接把 sampled latent cache 当作 `x_t` 做常规 flow matching 会让 loss 下降但 48-sample sequence identity 从 Stage11E fallback 的约 0.441 降到约 0.218，说明这是错误训练目标。下一步应做 bounded latent sequence repair，而不是继续扩大这个训练方式。
