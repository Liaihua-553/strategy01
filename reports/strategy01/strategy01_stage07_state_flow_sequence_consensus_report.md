...
- Stage07 postprocess job：`1908277`。
- 依赖：`afterok:1908276`，即数据生产成功后才启动。
...
- 目的：不额外占用第二张 GPU，同时让数据生产结束后自动进入 AE latent + pilot 微调闭环。

### 9.7 数据生产失败诊断与 quota 修复

- 旧数据生产 job：`1908276`。
- 失败原因：`OSError: [Errno 122] Disk quota exceeded`，发生在创建 `data/strategy01/stage07_production/boltz_outputs` 时。
- 连带影响：旧 afterok job `1908277` 进入 `DependencyNeverSatisfied`，运行时间一直为 0；已取消。
- 诊断结果：`/data` 用户 quota 到达 hard limit；Stage07 失败残留 `stage07_production` 约 3.7GB，其中 cache 约 3.5GB。
- 修复动作：删除失败残留 `data/strategy01/stage07_production`，释放约 3.7GB；新增 `stage06_build_boltz_production_dataset.py --work-dir` 参数，把 RCSB cache、seed states、Boltz outputs 全部写到计算节点 `/tmp/kfliao_strategy01_stage07_$SLURM_JOB_ID`，策略仓只保留最终 tensor/manifest/summary。
- 重新提交：新数据生产 job `1908293`，新 afterok 后处理 job `1908294`。
- 当前状态：`1908293` 已在 gu02 运行，早期日志无 quota 报错，`data/strategy01/stage07_production` 保持小体积，符合预期。
